...
class CommonFunc {
  ...
  Future<Uint8List> getBytesFromAsset(String path, double devicePixel) async {
    ByteData data = await rootBundle.load(path);
    ui.Codec codec = await ui.instantiateImageCodec(data.buffer.asUint8List(), targetWidth: (40*devicePixel).floor());
    ui.FrameInfo fi = await codec.getNextFrame();
    return (await fi.image.toByteData(format: ui.ImageByteFormat.png)).buffer.asUint8List();
  }

  Future<Uint8List> getBytesFromCanvas(urlAsset) async {
  ...
  static String formatMoney(dynamic number) {
    try {
      if (number.runtimeType != 1.0.runtimeType) {
